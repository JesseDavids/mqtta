    ...
    def process(self):
        ...
        while True:
            ...
            if(subT == ""):
                ...
                cmd = (f"Command=$(zenity --info --title={title} --text='Sent from {sender1} on {dt_string}\n\nMessage: {message1}' --width=300 --height=150); echo $Command")
                msg_result = subprocess.check_output([cmd], shell=True)
                
                
                client.publish(f"workstation/{hostname}/n/message", str("Okay"), 2, False)
